...
const EventEdit = () => {
  ...
  const { id } = useParams();
  const { data: eventData, isLoading: isLoadingEvent } = useGetEventQuery(id);
  ...
  useEffect(() => {
    if (eventData) {
      ...
      if (eventData.coverUrl) {
        setCoverImagePreview(eventData.coverUrl);
      }
    }
  }, [eventData, reset]);
    ...
    <div className="min-h-screen p-1">
      <div className="max-w-full mx-auto bg-white shadow-lg rounded-lg p-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          >
            {isSubmitting || isUpdating ? "Updating..." : "Update Event"}
          </Button>
          </div>
        </form>
      </div>
